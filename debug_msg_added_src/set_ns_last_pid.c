...
{
    char buf[64];
    warnx("MYCOMMENT: Enter set_ns_last_pid_privileged\n");
    ...
    warnx("MYCOMMENT: Finish set_ns_last_pid_privileged\n");
    return 0;
}
...
static int set_ns_last_pid(pid_t pid)
{
    if (!fork_hack_enabled) {
        ...
        warnx("MYCOMMENT: insufficient perm !fork_hack_enable\n");

        if (enable_fork_hack() == -1)
            return -1;
    }
    warnx("MYCOMMENT: Going to call fork hack for pid %d\n", pid);
...
static int run_server(const char* socket_path)
{
    ...
    warnx("MYCOMMENT: Enter run_server\n");
    ...
    warnx("MYCOMMENT: Begin server loop\n");
    for (;;) {
        uint32_t pid;
        ...
        warnx("MYCOMMENT: Begin set_ns_last_pid\n");
        char r = set_ns_last_pid((pid_t)pid) == -1 ? 1 : 0;
        warnx("MYCOMMENT: End set_ns_last_pid\n");
