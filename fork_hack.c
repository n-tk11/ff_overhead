...
int set_ns_last_pid_fork_hack(pid_t pid)
{
    ...
    warnx("MYCOMMENT: Begin loop fork hack for pid %d\n", pid);
    for (int pass = 1;; pass++) {
        current_pid = get_ns_last_pid();
        warnx("MYCOMMENT: In loop fork hack(for pid %d) current pid = %d (%ld)\n", pid, current_pid, time(NULL));
        if (current_pid == -1)
            return -1;
    ...
    warnx("MYCOMMENT: End loop fork hack\n");
